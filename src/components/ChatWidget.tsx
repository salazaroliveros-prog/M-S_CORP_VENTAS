...
export const ChatWidget = () => {
  ...
  // Migración: Inicializar o buscar sesión de chat en PostgreSQL/Neon
  React.useEffect(() => {
    if (!chatId && isOpen) {
  ...
  // Migración: Leer mensajes y estado de chat desde PostgreSQL/Neon
  React.useEffect(() => {
    if (!chatId) return;
    let polling = true;
    const fetchMessages = async () => {
      ...
      if (polling) setTimeout(fetchMessages, 3000); // Polling cada 3s
    };
    fetchMessages();
